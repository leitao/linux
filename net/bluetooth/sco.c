			       ...
			       sockopt_t *opt)
{
	...
	case BT_VOICE:
		...
		if (copy_to_iter(&voice, len, &opt->iter) != len)
			err = -EFAULT;
	...
	case BT_CODEC:
		...
		if (copy_to_iter(&num_codecs, sizeof(num_codecs), &opt->iter) !=
		    sizeof(num_codecs)) {
			hci_dev_put(hdev);
			return -EFAULT;
		}
		...
		list_for_each_entry(c, &hdev->local_codecs, list) {
			...
			if (copy_to_iter(&codec, sizeof(codec), &opt->iter) !=
			    sizeof(codec)) {
				err = -EFAULT;
				break;
			}
			...
			if (caps_len &&
			    copy_to_iter(c->caps, caps_len, &opt->iter) != caps_len) {
				err = -EFAULT;
				break;
			}
		}
