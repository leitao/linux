			       ...
			       sockopt_t *opt)
{
	...
	switch (optname) {
	case NFC_LLCP_RW:
		val = llcp_sock->rw > LLCP_MAX_RW ? local->rw : llcp_sock->rw;
		break;

	case NFC_LLCP_MIUX:
		val = be16_to_cpu(llcp_sock->miux) > LLCP_MAX_MIUX ?
			be16_to_cpu(local->miux) : be16_to_cpu(llcp_sock->miux);
		break;

	case NFC_LLCP_REMOTE_MIU:
		val = llcp_sock->remote_miu > LLCP_MAX_MIU ?
				local->remote_miu : llcp_sock->remote_miu;
		break;

	case NFC_LLCP_REMOTE_LTO:
		val = local->remote_lto / 10;
		break;

	case NFC_LLCP_REMOTE_RW:
		val = llcp_sock->remote_rw;
		break;
