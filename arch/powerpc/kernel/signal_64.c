...
SYSCALL_DEFINE0(rt_sigreturn)
{
	 ...
	 * not recheckpoint _this_ task if there was a context switch inside
	 * the exception.
	 *
	 ...
	 * MSR[TS] = Suspended (switch_to() calls tm_recheckpoint() when
	 * scheduling the 'new' task).  In this case, the process continues
	 * to be in the same in the CPU, but the CPU state just changed.
	 *
	 * This can cause a TM Bad Thing, since the MSR in the exception
	 * stack will have MSR[TS]=0, and this is what will be used to
	 * RFID.
	 *
	 * Clearing MSR[TS] state here will avoid a recheckpoint if there
	 * is any process reschedule in kernel space. The MSR[TS] state
	 * does not need to be saved also, since it will be replaced with
	 * the MSR[TS] that came from user context at
