...
static const char * const bench_scopes[] = {
	"cpu", "smt", "cache", "numa", "system",
};

static const unsigned int cache_shard_sizes[] = { 4, 8, 16, 32 };
...
static int __init test_workqueue_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(cache_shard_sizes); i++) {
		unsigned int sz = cache_shard_sizes[i];
		int ret = workqueue_update_cache_shard_size(sz);

		if (ret) {
			pr_err("test_workqueue: failed to set shard size %u: %d\n",
			       sz, ret);
			continue;
		}
		snprintf(label, sizeof(label), "cache_shard/%u", sz);
		run_bench(n_threads, "cache_shard", label);
	}
