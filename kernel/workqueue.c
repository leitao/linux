...
static unsigned int wq_cache_shard_count = 8;
module_param_named(cache_shard_count, wq_cache_shard_count, uint, 0444);
 ...
 * divided into at most wq_cache_shard_count shards, always split on core (SMT
 * group) boundaries so that SMT siblings are never placed in different shards.
 * Cores are distributed across shards as evenly as possible.
 *
 * Example: 10 cores (2 threads each) with max shard count 4 gives
 * 4 shards of 3+3+2+2 cores.
 */
static int __init cpu_cache_shard_id(int cpu)
{
	...
	/* Cap shard count at the number of cores (each shard needs at least one) */
	nr_shards = min(wq_cache_shard_count, (unsigned int)nr_cores);
...
void __init workqueue_init_topology(void)
{
	...
	if (!wq_cache_shard_count) {
		pr_warn("workqueue: cache_shard_count must be > 0, setting to 1\n");
		wq_cache_shard_count = 1;
	}
