...
"""
    Auto generte the documentation for Netlink specifications.

    :copyright:  Copyright (C) 2023  Breno Leitao <leitao@debian.org>
    :license:    GPL Version 2, June 1991 see linux/COPYING for details.

    This script performs extensive parsing specific to the Linux kernel's
    standard netlink specs, in an effort to avoid needing to heavily mark up
    the original YAML file.

    This code is split in three big parts:
        1) RST formatters: Use to convert a string to a RST output
        2) Parser helpers: Functions to parse the YAML data
...
def sanitize(text: str) -> str:
    ...
    # This is useful for some fields that are spread accross multiple lines
...
def generate_index(files: List[str], index_output: str) -> None:
    """Generates the `networking_spec/index` content and write to the file"""
    lines = []

    lines.append(rst_header())
    lines.append(rst_title("Netlink Specification"))
    lines.append(rst_toctree())

    for filename in files:
        lines.append(f"   {filename}\n")

    logging.debug("Writing an index at %s for files: %s", index_output, files)
...
def main() -> None:
    ...
    generate_index(processed_files, os.path.join(args.output_dir, "index.rst"))
